$("#form_compila").ready(function () {
    ...
    var min = data.getFullYear() + '-' + ("0" + (data.getMonth() + 1)).slice(-2) + '-' + ("0" + data.getDate()).slice(-2);
...
function sendGiorno(nCommessa, datagiorno, ore, tipoOra) {
    var Giorno = {};
    Giorno.Data = datagiorno
    Giorno.Ore = ore
    Giorno.Commessa = nCommessa
    Giorno.TipoOre = 'Ore di lavoro';
    $.post('../api/Getime', Giorno, function (data) {
        $(document).trigger("SuccessCompilaGiorno", ['Il giorno ' + datagiorno + " sono state aggiunte " + ore]);
    }).fail(function (data) {
        $(document).trigger("FailCompilaGiorno", [data.responseJSON.Message, datagiorno]);
    });
}